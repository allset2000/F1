...
-- Get the jobtype, documentid and MRN from previous record in that group or get it from jobs and patient table.
		SELECT TOP 1 JH.JobNumber,
				...
				un.UserId,
				CASE WHEN mr.MRN IS NULL THEN JP.MRN ELSE  mr.MRN END MRN,JP.FirstName,JP.MI,JP.LastName,jb.ClinicID,JH.JgId,IsError, NULL AS AppointmentDate, NULL AS DOB --added for 5366, to have correct columns returned to parent table
