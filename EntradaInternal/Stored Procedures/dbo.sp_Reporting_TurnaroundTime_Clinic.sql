...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Reporting_TurnaroundTime_Clinic]
--declare
	@ReceivedOn datetime,
	@ReceivedOn2 datetime,
	@PayTypeVar varchar(20) --- This is actually the ClinicCode

--set @ReceivedOn = '2013-05-21'
--set @ReceivedOn2 = '2013-05-26'
--set @PayTypeVar = 'BNJ'
...
If @receivedon = cast('1/1/1901' as datetime) or @receivedon2= cast('1/1/1901' as datetime)
	Begin
		Set @ReceivedOn = DATEADD(day,DATEDIFF(day,0,GETDATE())-1,0)
...
IF @PayTypeVar = 'QID'
	BEGIN
		SELECT C.ClinicName as [Clinic],
			...
			WHERE J.CompletedOn BETWEEN dateadd(dd, datediff(dd, 0, @ReceivedOn)+0, 0)  AND dateadd(dd, datediff(dd, 0, @ReceivedOn2)+1, 0) AND
				C.ClinicID IN (8, 14, 22, 31, 32, 36, 41)
	...
	BEGIN
		SELECT C.ClinicName as [Clinic],
			...
			LEFT OUTER JOIN [Entrada].[dbo].Jobs J2 WITH(NOLOCK) ON
				...
				AND (CAST((CAST(DATEDIFF(MI, J.ReceivedOn, J.CompletedOn) as DECIMAL(10,3)) / CAST(60 as DECIMAL(10,3))) AS DECIMAL(10,3)) < 24)
			WHERE J.CompletedOn BETWEEN dateadd(dd, datediff(dd, 0, @ReceivedOn)+0, 0)  AND dateadd(dd, datediff(dd, 0, @ReceivedOn2)+1, 0) AND
				(C.ClinicCode = @PayTypeVar) -- or C.ClinicID=@PayTypeVar)
			GROUP BY C.ClinicName,
					E.EditorID,
					D.LastName + ', ' + D.FirstName,
					E.LastName + ', ' + E.FirstName,
