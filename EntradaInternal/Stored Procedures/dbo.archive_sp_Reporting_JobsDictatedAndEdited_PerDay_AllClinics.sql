...
--				for report "Jobs Dictated and Edited Per Day (All Clinics).rdl"
--
-- Edits:
-- 2/4/13	jablumenthal	added DictatorID per Jeff McNeese request
-- =============================================
create PROCEDURE [dbo].[archive_sp_Reporting_JobsDictatedAndEdited_PerDay_AllClinics]
...
IF @PayTypeVar = 'QID'
	BEGIN
		...
		WHERE (J.CompletedOn BETWEEN dateadd(dd, datediff(dd, 0, @BeginDate)+0, 0)  AND dateadd(dd, datediff(dd, 0, @EndDate)+1, 0)) 
			--(J.ReturnedOn BETWEEN dateadd(dd, datediff(dd, 0, @BeginDate)+0, 0)  AND dateadd(dd, datediff(dd, 0, @EndDate)+1, 0)) 
