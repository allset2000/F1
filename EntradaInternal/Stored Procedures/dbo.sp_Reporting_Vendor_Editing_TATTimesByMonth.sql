...
Create date:	8/22/14
Description:	Returns Total Vendor Editor TAT Times 
======================================================= */
	
	CREATE PROCEDURE [dbo].[sp_Reporting_Vendor_Editing_TATTimesByMonth]
...
VendorName, MonthYear
      ,COUNT(CASE WHEN TimeDifference <= 480 THEN 1 END) As InTAT
      ,COUNT(CASE WHEN TimeDifference > 480 THEN 1 END) As OutofTAT
      ,(CAST(COUNT(CASE WHEN TimeDifference <= 480 THEN 1 END)as Float) / CAST(COUNT(JobNumber) as Float))  AS InTATPercent
      ,(CAST(COUNT(CASE WHEN TimeDifference > 480 THEN 1 END)as Float) / CAST(COUNT(JobNumber) as Float))  AS OutOfTATPercent
