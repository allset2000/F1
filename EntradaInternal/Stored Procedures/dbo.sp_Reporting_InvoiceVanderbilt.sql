...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Reporting_InvoiceVanderbilt]
	@BeginDate DATETIME,
	@EndDate DATETIME
...
SELECT	C.ClinicName AS [Clinic], 
			...
			CASE WHEN J.Stat = 0 THEN COALESCE(CAST(BJ.[DocumentWSpaces] AS DECIMAL(10,2)), 0) / 65.0 ELSE 0 END AS [Normal Units],
			...
			CASE WHEN J.Stat = 0 THEN COALESCE(CAST(BJ.[DocumentWSpaces] AS DECIMAL(10,2)), 0) / 65.0 ELSE 0 END * (DT.decDocTechRate + DT.decDocEditRate) AS [Normal Amt],
			...
			CASE WHEN J.Stat = 1 THEN COALESCE(CAST(BJ.[DocumentWSpaces] AS DECIMAL(10,2)), 0) / 65.0 ELSE 0 END AS [Urgent Units],
			...
			CASE WHEN J.Stat = 1 THEN COALESCE(CAST(BJ.[DocumentWSpaces] AS DECIMAL(10,2)), 0) / 65.0 ELSE 0 END * (DT.decDocTechRate + DT.decDocEditRate + COALESCE(DT.decDocStatSurcharge, 0)) AS [Urgent Amt],
	...
	FROM [Entrada].[dbo].Clinics C WITH(NOLOCK)
		...
		INNER JOIN [Entrada].dbo.vwRptBillableJobs BJ WITH (NOLOCK) ON
