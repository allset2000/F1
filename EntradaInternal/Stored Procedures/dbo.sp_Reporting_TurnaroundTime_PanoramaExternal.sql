...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Reporting_TurnaroundTime_PanoramaExternal]
...
SET NOCOUNT ON
	...
	BEGIN
		...
		-- Get Externally Editted Dictators
		SELECT C.ClinicName as [Clinic],
			...
			WHERE --J.CompletedOn BETWEEN dateadd(dd, datediff(dd, 0, GETDATE())-1, 0)  AND dateadd(dd, datediff(dd, 0, GETDATE())+0, 0) AND
				...
				D.DictatorID NOT IN (SELECT DictatorID FROM [EntradaInternal].[dbo].PanDictatatorInternalEdit) AND
